...
async fn dll_replace(path: &str, app: AppHandle) -> Result<bool, String> {
    ...
    match download_result {
        Ok(_) => {
            println!("Successfully downloaded Starfall.dll");
            Ok(true)
        }
        Err(e) => {
            println!("Failed to download Starfall.dll: {}", e);
            Ok(true)
        }
    }
}
...
pub async fn launch_fn(
    ...
    // Replace DLL (inject Starfall.dll)
    match dll_replace(path, app.clone()).await {
        Ok(_) => {}
        Err(e) => return Err("Could not launch the game: ".to_string() + &e),
    }
    ...
    if launch_real_launcher(base.to_str().unwrap()).await.is_err() {
    ...
    let game_pid = fort_cmd.unwrap().id();
    println!("Fortnite launched successfully (PID: {})", game_pid);

    // Suspend stuff pablo cant do patched by drico ud crazy sigma tuff strong brazilian boy
    tokio::spawn(async move {
        ...
        for proc in sys.processes().values() {
            let name = proc.name();
            if name == "FortniteClient-Win64-Shipping_EAC.exe" || name == "FortniteClient-Win64-Shipping_BE.exe" {
                let pid = proc.pid().as_u32();
                let (count, err) = suspend_process(pid);
                if !err && count > 0 {
                    println!("Suspended {} (PID: {}) – {} threads", name.to_string_lossy(), pid, count);
                }
            }
        }
    });
